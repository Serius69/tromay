            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Venta de Divisa</h4>
                ...
                <div class="col-xxl-7 col-xl-8 col-lg-10">
                    ...
                    <div class="card">
                        <div class="card-header d-flex align-items-center">
                            <span class="badge bg-warning text-dark me-2 fs-13">VENTA</span>
                            <h5 class="card-title mb-0">El cliente entrega BOB — recibe divisa</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="row g-3 mb-3">
                                {{-- Origin currency (cash1 = what client gives = BOB side) --}}
                                <div class="col-md-6">
                                    <label class="form-label" for="cash1_id">
                                        Moneda que entrega el cliente <span class="text-danger">*</span>
                                    </label>
                                ...
                                {{-- Foreign currency (cash2 = what client receives) --}}
                                <div class="col-md-6">
                                    ...
                                    <select id="cash2_id" name="cash2_id" class="form-select" required>
                                        ...
                                        <option value="{{ $cash->id }}"
                                            ...
                                            data-name="{{ strtoupper($cash->getRawOriginal('name')) }}">
                                            {{ strtoupper($cash->getRawOriginal('name')) }}
                                            &mdash; Venta: {{ number_format($cash->sell, 4) }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label" for="amount1">
                                        Monto entregado por el cliente <span class="text-danger">*</span>
                                    </label>
                            ...
                            {{-- Live rate + preview --}}
                            <div id="rate-preview" class="alert alert-soft-warning border-warning d-none mb-4">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <span class="text-muted fs-12">Tasa de venta</span>
                                        <div class="fw-semibold fs-15" id="preview-rate">—</div>
                                    </div>
                                    <div class="text-end">
                                        <span class="text-muted fs-12">El cliente recibirá aprox.</span>
                                        <div class="fw-bold fs-18 text-warning" id="preview-amount2">—</div>
                                    </div>
                                </div>
                            ...
                            {{-- Submit --}}
                            <div class="text-end">
                                <button type="button" id="btn-open-confirm" class="btn btn-warning btn-lg px-5 text-dark" disabled>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-warning text-dark">
                <h5 class="modal-title">Confirmar operación de venta</h5>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning text-dark" id="btn-confirm-submit">
...
(function () {
    ...
    const TX_TYPE    = 1; // TYPE_BUY: client buys foreign currency, pays BOB
    ...
    // ----------------------------------------------------------------
    // Rate preview (TYPE_BUY: amount2 = amount1 / cash2.sell)
    // ----------------------------------------------------------------
    function updatePreview() {
        ...
        // La casa VENDE divisa → aplica la tasa de venta (cashes.sell).
        const rateVal  = cash2Opt ? parseFloat(cash2Opt.dataset.sell) || 0 : 0;
        const currency = cash2Opt ? cash2Opt.dataset.name : '';
        ...
        if (rateVal > 0 && amount1 > 0 && currency) {
            ...
            document.getElementById('preview-rate').textContent =
                '1 ' + currency + ' = ' + fmt(rateVal, 4) + ' BOB (tasa venta)';
            document.getElementById('preview-amount2').textContent =
                fmt(amount2, 4) + ' ' + currency;
    ...
    document.getElementById('cash2_id').addEventListener('change', updatePreview);
            ...
            <div class="table-responsive">
                <table class="table table-sm mb-0">
                    <tbody>
                        ...
                        <tr><th class="text-muted fw-normal">Moneda entregada</th><td>${esc(cash1Opt?.dataset.name ?? '—')}</td></tr>
                        <tr><th class="text-muted fw-normal">Divisa recibida</th><td>${esc(cash2Opt?.dataset.name ?? '—')}</td></tr>
                        <tr><th class="text-muted fw-normal">Monto entregado</th><td><strong>${fmt(amount1, 2)}</strong></td></tr>
                        <tr><th class="text-muted fw-normal">Tasa de venta</th><td>${fmt(rateVal, 4)}</td></tr>
                        <tr class="table-warning"><th class="fw-semibold">Divisa a recibir (aprox.)</th><td><strong>${fmt(amount2, 4)} ${esc(cash2Opt?.dataset.name ?? '')}</strong></td></tr>
